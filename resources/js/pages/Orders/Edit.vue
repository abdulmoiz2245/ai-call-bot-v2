<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                <h1 class="text-3xl font-bold tracking-tight">Edit Order</h1>
                <p class="text-muted-foreground">
                Update order {{ order.order_number }}
                </p>
            </div>
            <div class="flex items-center space-x-2">
                <Button @click="$inertia.visit(route('orders.show', order.id))" variant="outline">
            ...
            <CardContent>
                ...
                <div class="flex items-center justify-end space-x-4">
                    <Button @click="$inertia.visit(route('orders.show', order.id))" type="button" variant="outline">
                    Cancel
                    </Button>
...
import { ArrowLeft, Eye, Plus, Trash2, Package, Loader2 } from 'lucide-vue-next'
...
const calculateOrderTotal = () => {
  const total = form.items.reduce((sum: number, item: any) => {
    const quantity = parseFloat(String(item.quantity)) || 0
    const unitPrice = parseFloat(String(item.unit_price)) || 0
